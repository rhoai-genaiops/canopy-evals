"""
Simplified Kubeflow Pipeline for Automated Canopy Testing

This pipeline implements an automated testing workflow that:
1. Scans directories for LLM Test configuration (assumes git repo already cloned by Tekton)
2. Executes the LLM tests
"""
...
@dsl.pipeline(
    name="Simple Canopy Testing Pipeline",
    description="Simplified pipeline for running canopy tests (git clone handled by Tekton)"
)
def simple_canopy_test_pipeline(
    ...
    # Step 2: Run all llamastack tests
